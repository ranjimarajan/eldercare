...
router.get("/getAllCare", async (req, res) => {
  try {
    const getAllCare = await careGiverModel.find();
    console.log(getAllCare,"getAllCare")
    res.json(getAllCare)
  } catch (error) {
    console.log(error);
    // res.status(400).send({ error: "Something went wrong" });
...
router.post("/addCareGiver", async (req, res) => {
  try {
    console.log('Received Data:', req.body);

    const care = new careGiverModel(req.body);
    await care.save();

    res.status(200).send({ message: "Caregiver added successfully", care });
  } catch (error) {
    console.error("Error:", error);
    res.status(400).send({ error: "Something went wrong" });
  }
});
router.post("/Carelogin", async (req, res) => {
  ...
  try {
    ...
    res.status(200).send({ message: "Login successful", caregiver });
  } catch (error) {
